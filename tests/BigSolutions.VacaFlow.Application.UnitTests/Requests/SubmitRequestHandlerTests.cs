...
public sealed class SubmitRequestHandlerTests
{
    ...
    private SubmitRequestHandler CreateHandler(
    ...
    [Fact]
    public async Task Handle_Should_Succeed_When_The_Owner_Submits_Their_Own_Draft()
    {
        ...
        Assert.Equal(RequestState.Submitted, draft.State);
        Assert.Equal(FixedNow.UtcDateTime, draft.SubmittedAtUtc);
    }
    ...
    /// <summary>
    /// A non-owner of a request that is no longer a Draft still gets 403,
    /// not 409 — ownership is decided before the transition itself (S1).
    /// </summary>
    [Fact]
    public async Task Handle_Should_Fail_With_Not_Owner_Even_When_The_Request_Is_Already_Submitted()
    {
        var draft = NewDraft();
    ...
    [Fact]
    public async Task Handle_Should_Fail_When_The_Start_Date_Has_Since_Passed()
    {
        var draft = NewDraft();
        var requests = new FakeRequestRepository(draft);
        var oneDayLater = FixedNow.AddDays(1);
        var handler = CreateHandler(requests, now: oneDayLater);
    ...
    [Fact]
    public async Task Handle_Should_Fail_When_The_Request_Is_Already_Submitted()
    {
        var draft = NewDraft();
        draft.Submit(Today, FixedNow.UtcDateTime);
        var requests = new FakeRequestRepository(draft);
